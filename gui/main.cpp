...
struct AppState {
  // User input
  int birth_year = 1984; // Matches Sprind-Funke mdoc
  int birth_month = 9;
  int birth_day = 1;
  ...
  bool prove_student_status = false;

  // Health Pass / Issuer Settings
  bool prove_health_issuer = true;
  bool prove_vaccine = true;
...
void GenerateZKProofAsync(AppState &state) {
  ...
  config.prove_student_status = state.prove_student_status;
  ...
  state.generation_task = std::async(std::launch::async, [&state, config]() {
    ...
    bool success =
        PerformZKProofGeneration(config, result_proof, log_buffer, age_out);

    {
      std::lock_guard<std::recursive_mutex> lock(state.mutex);
      // Merge log
      state.log += log_buffer;
      // Note: age_out returned by prover is ignored for state, UI recalculates
      // it if needed.
...
void RenderMainWindow(AppState &state) {
  ...
  ImGui::BeginChild("Header", ImVec2(0, 60), false);
  {
    ...
    ImGui::PushFont(
        ImGui::GetIO().Fonts->Fonts[0]); // Use default font (scaled if needed)
  ...
  if (ImGui::BeginTabBar("MainTabs")) {
    ...
    if (ImGui::BeginTabItem("Identity Verification")) {
      ...
      state.prove_student_status = false;
      state.prove_health_issuer =
          false; // Fix: Disable health issuer mode when in Identity tab
      ImGui::Spacing();
      ImGui::TextWrapped(
          "Verify age or nationality using a trusted mDoc credential, without "
          "revealing your full birth date or ID number.");
      ImGui::Spacing();
      ImGui::Separator();
      ImGui::Spacing();

      ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.15f, 0.16f, 0.20f, 1.0f));
      if (ImGui::CollapsingHeader("User Profile (Private Input)",
        ...
        if (ImGui::BeginTable("DateInputTable", 4)) {
          ...
          ImGui::PopItemWidth();
          if (ImGui::IsItemHovered())
            ImGui::SetTooltip("Day (1-31)");

          ImGui::EndTable();
        }

        // Calculated locally for display only, not stored in state
        int age =
        ...
        if (state.prove_nationality) {
          ...
          if (nation_items.empty()) {
            for (int i = 0; i < kNumCountries; ++i) {
              ...
              std::string label = std::string(c.name) + " (" + c.alpha3 +
                                  " / " + c.numeric + ")";
              nation_items.push_back(label);
            }
            for (const auto &s : nation_items)
              nation_ptrs.push_back(s.c_str());
          }

          ImGui::Combo("##NatCombo", &state.selected_nationality,
                       nation_ptrs.data(), (int)nation_ptrs.size());
        }
        ImGui::Unindent(10);
        ImGui::Spacing();
      }
    ...
    if (ImGui::BeginTabItem("Issuer Verification")) {
      ...
      ImGui::TextColored(ImVec4(0.2f, 0.8f, 0.4f, 1.0f),
                         "Health Certificate Issuer Verification");

      ImGui::TextWrapped(
          "Verify the Issuing Authority of a digital health certificate using "
          "Zero-Knowledge Proofs, without revealing personal health data.");
      ...
      if (ImGui::CollapsingHeader("Certificate Authority",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        const char *items[] = {"France (Ministère de la Santé)", "USA (CDC)",
                               "Deutschland (RKI)", "Invalid / Other"};

        ImGui::Combo("##IssuerCombo", &state.selected_issuer, items,
                     IM_ARRAYSIZE(items));

        ImGui::Spacing();

        ImGui::Checkbox("EU Mandatory Vaccines Compliant",
                        &state.eu_vaccines_compliant);
        ImGui::SameLine();
        ImGui::TextDisabled("(?)");
        if (ImGui::IsItemHovered()) {
          ImGui::BeginTooltip();
          ...
          ImGui::TextUnformatted(
              "Verifies compliance with all mandatory vaccines for EU travel.");
          ImGui::PopTextWrapPos();
          ImGui::EndTooltip();
        }

        ImGui::Spacing();
        ImGui::Checkbox("Verify Specific Vaccine (Comirnaty/Pfizer)",
                        &state.prove_vaccine);
        ImGui::Checkbox("Verify Health Insurance Status",
                        &state.prove_insurance);
        ...
        ImGui::TextWrapped(
            "The selected authority will be verified against the signed "
            "document using Zero-Knowledge proofs. No local simulation.");
      ...
      if (ImGui::CollapsingHeader("ZK Verification Scope",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        ImGui::TextDisabled(
            "Note: Personal identity (Name, DOB) is NOT revealed.");
    ...
    if (ImGui::BeginTabItem("Driver's License")) {
      ...
      ImGui::TextWrapped("Verify that you hold a valid French Driver's License "
    ...
    // TAB 4: Student ID
    if (ImGui::BeginTabItem("Student ID")) {
      state.prove_student_status = true;
      state.prove_french_license = false;
      state.prove_health_issuer = false;
      state.prove_age = false;
      state.prove_nationality = false;
      state.prove_sex = false; // Reset sex proof

      ImGui::Spacing();
      ImGui::TextColored(state.accent_color, "Student Status Verification");
      ImGui::TextWrapped("Verify active student status using the Ministry of "
      ...
      ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.15f, 0.16f, 0.20f, 1.0f));
      if (ImGui::CollapsingHeader("Education Credentials",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
        ImGui::Indent(10);
        ImGui::Spacing();

        ImGui::Checkbox("Verify 'is_student' attribute",
                        &state.prove_student_status);
        ImGui::SameLine();
        ImGui::TextDisabled("(?)");
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip("Checks for the presence of the signed "
                            "'is_student' boolean attribute.");
        }

        ImGui::Spacing();
        ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.2f, 1.0f), "Note:");
        ImGui::TextWrapped("Requires a valid mDoc signed by the Ministry of "
                           "Education. (Not available in test data)");
    ...
    if (ImGui::BeginTabItem("Gender Verification")) {
      ...
      ImGui::TextWrapped("Prove your sex/gender using Zero-Knowledge proofs "
                         "without revealing other personal information.");
      ...
      ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.15f, 0.16f, 0.20f, 1.0f));
      if (ImGui::CollapsingHeader("Sex Selection",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
        ImGui::Indent(10);
        ImGui::Spacing();
        ...
        ImGui::Spacing();
        ImGui::Combo("##SexCombo", &state.selected_sex, sex_options,
                     IM_ARRAYSIZE(sex_options));
        ...
        ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f),
                           "Privacy Guarantee:");
        ImGui::TextWrapped(
            "The ZK proof will only reveal that your sex matches the "
            "selected value. No other personal data (name, birth date, "
            "document number) is disclosed.");

        ImGui::Spacing();
        ImGui::TextDisabled("* Note: Only binary sex values (M/F) are "
                            "supported as per ISO 18013-5 standard");

        ImGui::Spacing();
        ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f),
                           "[WARNING] Test mDoc missing 'sex' attribute.");
        ImGui::TextWrapped("The current test document does not contain the "
                           "'sex' attribute. Proof generation will fail.");
  ...
  if (state.is_generating) {
    ...
    ImGui::Button("GENERATING...",
                  ImVec2(ImGui::GetContentRegionAvail().x, button_height));
    ImGui::PopStyleColor();
    ImGui::PopItemFlag();
  } else {
    ...
    if (ImGui::Button("GENERATE PROOF", ImVec2(ImGui::GetContentRegionAvail().x,
                                               button_height))) {
  ...
  ImGui::BeginChild("StatusBar", ImVec2(0, 80), false);
  {
    ...
    if (state.status_message.empty()) {
      ...
      if (state.status_message.find("✓") != std::string::npos ||
          state.status_message.find("[OK]") != std::string::npos)
        statusColor = ImVec4(0.4f, 1.0f, 0.4f, 1.0f);
      else if (state.status_message.find("fail") != std::string::npos ||
               state.status_message.find("Error") != std::string::npos ||
               state.status_message.find("❌") != std::string::npos)
...
int main(int, char **) {
  // Note: Log level might be handled inside zk_workflow if needed, or here
  // globally
