...
struct AppState {
  ...
  // Note: The internal structure is hidden in zk_workflow.cpp, here we just
  // hold byte buffers or similar if we wanted strict separation but for now we
  // keep the definition compatible with what logic expects (void* casting)
  struct CircuitCache {
  ...
  if (ImGui::BeginTabBar("MainTabs")) {
    ...
    if (ImGui::BeginTabItem("Driver's License")) {
      ...
      if (ImGui::CollapsingHeader("Attributes to Verify",
        ...
        ImGui::Spacing();
        ImGui::TextDisabled(
            "* Demo Note: Uses 'height' as proxy for B-Category");
        ImGui::Unindent(10);
        ImGui::Spacing();
      }
      ImGui::PopStyleColor();
