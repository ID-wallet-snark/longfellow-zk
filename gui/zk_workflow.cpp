...
const CountryEntry kCountries[] = {
    {"France", "FRA", "250"},      {"United States", "USA", "840"},
    {"Germany", "DEU", "276"},     {"United Kingdom", "GBR", "826"},
    {"Spain", "ESP", "724"},       {"Italy", "ITA", "380"},
    {"Poland", "POL", "616"},      {"Netherlands", "NLD", "528"},
    {"Belgium", "BEL", "056"},     {"Sweden", "SWE", "752"},
    {"Switzerland", "CHE", "756"}, {"Austria", "AUT", "040"},
    {"Portugal", "PRT", "620"}};
...
bool ExportProof(const ProofData &proof_data, const ProverConfig &config,
  ...
  for (size_t i = 0; i < proof_data.attributes_proven.size(); ++i) {
    ...
    if (i < proof_data.attributes_proven.size() - 1)
      file << ",";
    file << "\n";
  }
  ...
  for (size_t i = 0; i < proof_data.zkproof.size(); ++i) {
    file << std::hex << std::setw(2) << std::setfill('0')
         << static_cast<int>(proof_data.zkproof[i]);
  }
  file << "\",\n";
  file << "  \"settings\": {\n";
...
// Defines strict cache structure to match what was in main.cpp, but anonymous
// here since we use void* in header
struct CircuitCache {
...
bool PerformZKProofGeneration(const ProverConfig &config, ProofData &proof_out,
  ...
  // Age Check
  int calculated_age =
      CalculateAge(config.birth_year, config.birth_month, config.birth_day);
  ...
  if (config.prove_age && calculated_age < config.age_threshold) {
    Log("  [BLOCKED] User is " + std::to_string(calculated_age) +
        ", but threshold is " + std::to_string(config.age_threshold));
    return false;
  }
  ...
  try {
    ...
    if (config.prove_french_license) {
      ...
      if (config.selected_issuer == 0)
        target_issuer = "FRA";
      else if (config.selected_issuer == 1)
        target_issuer = "USA";
      else if (config.selected_issuer == 2)
        target_issuer = "DEU";
      else
        target_issuer = "INVALID";
    ...
    if (attributes.size() == 1)
      cache = (CircuitCache *)config.circuit_cache_1attr;
    else if (attributes.size() == 2)
      cache = (CircuitCache *)config.circuit_cache_2attr;
    ...
    if (!circuit_from_cache) {
      ...
      // Update cache if possible (requires mutex in caller usually, but here we
      // assume config passed valid ptrs)
      if (cache) {
    ...
    if (config.prove_french_license) {
      ...
      if (config.prove_category_B)
        proof_out.attributes_proven.push_back("Category B");
      if (config.prove_category_A)
        proof_out.attributes_proven.push_back("Category A");
      if (config.prove_category_C)
        proof_out.attributes_proven.push_back("Category C");
    } else if (config.prove_health_issuer) {
      proof_out.attributes_proven.push_back("Issuer Verified");
      std::string iss = (config.selected_issuer == 0)
                            ? "FRA"
                            : ((config.selected_issuer == 1) ? "USA" : "DEU");
...
bool PerformZKVerification(const ProofData &proof_data, std::string &log_out) {
  ...
  try {
    const proofs::MdocTests *test =
        &proofs::mdoc_tests[proof_data.mdoc_test_index];
    ...
    MdocVerifierErrorCode verifier_ret = run_mdoc_verifier(
        ...
        proof_data.zkproof.data(), proof_data.zkproof.size(), test->doc_type,
        zk_spec);
