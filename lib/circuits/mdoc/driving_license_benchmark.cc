...
// Benchmark for Circuit Generation (driving license - 1 attribute)
void BM_CircuitGeneration_DrivingLicense(benchmark::State &state) {
...
BENCHMARK(BM_CircuitGeneration_DrivingLicense);

// Benchmark for Prover (driving_privileges)
void BM_Prover_DrivingLicense(benchmark::State &state) {
  // Setup
  uint8_t *circuit = nullptr;
  size_t circuit_len = 0;
  ...
  // Use mdoc_tests[2] which is the canonical playground example
  const MdocTests *test_data = &mdoc_tests[2];

  // Use age_over_18 attribute from this document
...
BENCHMARK(BM_Prover_DrivingLicense);

// Benchmark for Verifier (driving_privileges)
void BM_Verifier_DrivingLicense(benchmark::State &state) {
  // Setup
  uint8_t *circuit = nullptr;
  size_t circuit_len = 0;
  if (generate_circuit(&kZkSpecs[0], &circuit, &circuit_len) !=
...
BENCHMARK(BM_Verifier_DrivingLicense);
