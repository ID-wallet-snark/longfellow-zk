...
// Benchmark for Circuit Generation (height - 1 attribute)
void BM_CircuitGeneration_Height(benchmark::State &state) {
...
BENCHMARK(BM_CircuitGeneration_Height);

// Benchmark for Prover (height)
void BM_Prover_Height(benchmark::State &state) {
  // Setup
  uint8_t *circuit = nullptr;
  size_t circuit_len = 0;
  ...
  // Use mdoc_tests[3] which contains height attribute
  const MdocTests *test_data = &mdoc_tests[3];

  RequestedAttribute attrs[] = {test::height_175};
...
BENCHMARK(BM_Prover_Height);

// Benchmark for Verifier (height)
void BM_Verifier_Height(benchmark::State &state) {
  // Setup
  uint8_t *circuit = nullptr;
  size_t circuit_len = 0;
  if (generate_circuit(&kZkSpecs[0], &circuit, &circuit_len) !=
