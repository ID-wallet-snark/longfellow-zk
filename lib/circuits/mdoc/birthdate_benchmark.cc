...
// Benchmark for Circuit Generation (birthdate - 1 attribute)
void BM_CircuitGeneration_Birthdate(benchmark::State &state) {
...
BENCHMARK(BM_CircuitGeneration_Birthdate);

// Benchmark for Prover (birthdate)
void BM_Prover_Birthdate(benchmark::State &state) {
  // Setup
  uint8_t *circuit = nullptr;
  size_t circuit_len = 0;
  ...
  // Use mdoc_tests[3] which contains birthdate attribute
  const MdocTests *test_data = &mdoc_tests[3];

  RequestedAttribute attrs[] = {test::birthdate_1971_09_01};
...
BENCHMARK(BM_Prover_Birthdate);

// Benchmark for Verifier (birthdate)
void BM_Verifier_Birthdate(benchmark::State &state) {
  // Setup
  uint8_t *circuit = nullptr;
  size_t circuit_len = 0;
  if (generate_circuit(&kZkSpecs[0], &circuit, &circuit_len) !=
...
BENCHMARK(BM_Verifier_Birthdate);
