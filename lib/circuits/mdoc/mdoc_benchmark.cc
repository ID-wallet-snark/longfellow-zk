...
BENCHMARK(BM_CircuitGeneration);
...
void BM_Prover(benchmark::State &state) {
  ...
  for (size_t i = 0; i < kMdocTestsCount; ++i) {
    if (std::string(mdoc_tests[i].doc_type) == "fr.gouv.education.1.student") {
      test_data = &mdoc_tests[i];
      break;
    }
  }

  if (!test_data) {
    state.SkipWithError("Student mDoc data not found");
    free(circuit);
    return;
  }
  ...
  for (auto _ : state) {
    // We pause timing to cleanup previous proof if needed, though
    // run_mdoc_prover allocates new one But we should ideally free it.
    state.PauseTiming();
    if (zkproof) {
      free(zkproof);
      zkproof = nullptr;
